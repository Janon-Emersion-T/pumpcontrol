<x-layouts.app :title="__('Add Pump')">
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl bg-white dark:bg-gray-900 p-6 shadow-lg">
        ...
        <form action="{{ route('pump.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="fuel_id" id="fuel_id" required class="input-field">
                    ...
                    @foreach ($fuels as $fuel)
                        <option value="{{ $fuel->id }}" @selected(old('fuel_id') == $fuel->id)>{{ $fuel->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                <button type="submit" class="btn-primary">Save Pump</button>
            </div>
        </form>
    </div>
</x-layouts.app>
